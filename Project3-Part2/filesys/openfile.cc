...
OpenFile::WriteAt(char *from, int numBytes, int position)
{
    ...
    // Extend the file size if needed to afford this write
    if ((position + numBytes) > fileLength)
    {
        int numBytesToExtendBy = position + numBytes - fileLength;
        int numSectorsToExtendBy = divRoundUp(numBytesToExtendBy, SectorSize);
        bool success = hdr->ExtendFile(numSectorsToExtendBy);
        if (!success)
        {
            printf("Not enough disk space to extend the file.\n");
            return 0;
        }
        int currPID = currentThread->space->getPCB()->getPID();
        printf("F %d: %d -> %d\n", currPID, fileLength, fileLength + (numSectorsToExtendBy * SectorSize));
    }

    fileLength = hdr->FileLength();              // refresh for updated fileLength value
    ASSERT(position < fileLength);               // we should have enough room now
    ASSERT((position + numBytes) <= fileLength);

    // Handle case where the write is within the bounds of the file
