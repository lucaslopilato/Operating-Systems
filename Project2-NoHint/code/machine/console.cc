...
Console::CheckCharAvail()
{
    ...
    nChars = ReadPartial(readFileNo, &c, sizeof(char));

    // if we didn't get anything we are probably reading from a
    // non-blocking fd so just return as if there was nothing to read
    if (nChars == 0)
        return;
